...
<div class="jumbotron">
    ...
    <p>The language interpreter runs in a cross platform IDE (OS X/Linux/Windows) and communicates via Open Sound Control via TCP or UDP with one or more synthesis servers.</p>

    <p>The SuperCollider synthesis server is runs in a separate process or even on a separate machine and can be connected to via TCP/UDP. So it is ideal for realtime networked music.</p>
